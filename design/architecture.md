...
| **4** | ~~Move the GUI obs worker onto the session layer.~~ **Done for the obs worker** — verified live against a real caster. The session gained NS_EV_RAW (non-RTCM format sniffing needs pre-framing bytes), ns_send_gga() (the VRS test features drive the uplink interactively) and ns_set_framing_enabled() (an SBF stream must not be run through the RTCM framer). The GUI's duplicate NtripHandshake and ParseNtripResponse() are deleted as promised. The replay and eph workers followed in a second pass, both verified live: replay runs the identical ObsOnEvent path as the live stream via ns_open_file(), and the eph worker became a 40-line handler over a second session — which also retired its lenient strstr("200") response check, a bug the obs-path fix had never reached. gui_thread.c: 1377 → 901 lines, zero transport code left. | Medium — behaviour must be compared before and after |
| **5** | ~~Move the CLI onto it.~~ **Done** — the five entry points are gone; `src/ntrip_handler.c` shrank from 1369 to 316 lines and now holds only the sourcetable fetch and pure helpers. The CLI modes live in `src/cli_stream.c` as event handlers, verified live against the same caster with before/after output capture: `-m` byte-identical bar the Date header, `-s` and `-d` format-identical, `-t` format-identical with corrected statistics (the legacy table could print an average below its own minimum). `--sky` still uses `main.c`'s own two framing loops — the last framing duplicates in the tree, left for a follow-up. | Medium |
| **6** | ~~Directory move and CMake.~~ **Done** — `src/` is now `core/`, `net/`, `session/`, `cli/`; 25 files moved with `git mv` so history follows them, and 27 files had includes rewritten. `gui/design.md` moved to `design/gui-design.md`. CMakeLists.txt builds `ntrip_core` and `ntrip_session` as libraries with the version parsed from `version.h`. Verified: all legacy build paths (build-gui.bat, both VS Code tasks, the documented Linux commands, service/Makefile) still work, and all three unit suites pass. CMake verified on both platforms (4.4.2 on Windows with the MinGW generator, 3.28 on Ubuntu): all five targets build, and the GUI's version resource comes out correct, so the version.h → windres chain works through CMake too. | Low but noisy in `git log` |
| **7** | Android JNI wrapper — now unblocked: `core/` and `session/` are isolated and CMake exists for the NDK to drive. | Independent of the rest |
