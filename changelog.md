...
`gui/ntrip-analyser.ico`: a ground station with arcs rising from it,
generated by script rather than drawn by hand.  Six sizes from 16 to
256, each rendered at its own scale rather than downsampled from one
large image — a 32×32 arc pattern squashed to 16 turns to mush, and 16
is the size the icon spends most of its life at.
